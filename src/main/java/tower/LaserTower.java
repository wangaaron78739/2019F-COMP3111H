...
public class LaserTower extends Tower {
	...
	public void shoot(){
		if(Arena.getMonsterNum() > 0 && Resource.canDeductAmount(10)){
			...
			double targetMonX = (map.entrySet().iterator().next().getKey().getXGrid() + 0.5) * GRID_WIDTH;
    		double targetMonY = (map.entrySet().iterator().next().getKey().getYGrid() + 0.5) * GRID_HEIGHT;
			...
			//if the attacking trace is not a horizontal line:
			if(targetMonY != towerY){
				double slopeReciprocal = (targetMonX - towerX)/(targetMonY - towerY); 
				double frameX = 0, frameY = 0;
				if(targetMonY > towerY){
					frameY = (MAX_H_NUM_GRID + 1) * GRID_HEIGHT;
					frameX = ((MAX_H_NUM_GRID + 1) * GRID_HEIGHT - towerY) * slopeReciprocal + towerX;
				}
				if(targetMonY <= towerY){
